...
## BPE TOKENIZER
The Byte Pair Tokenization is kind of combination of both word based and character based tokenzation. It settles in middle ground ie sub words, and the BPE algorithm does it so elegantly that it doesn't leave any room for a word that could actually suprise the model (if trained properly*).
BPE algorithm starts from every possible character in the dictionary (and this  applies to every language that can be rendered on a screen). But if we really think about it there are enough languages, and in turn characters in each language to overwhelm the tokenizer (As of Unicode version 16.0 (September 2024), the standard defines 154,998 unique characters). But as I said BPE  is more elegant than you think!!
The `B` in "BPE" handles these many characters easily.
BPE stands for "Byte Pair Encoding". The BYTE part handles each and every character. To understand this, we need to look at UTF/Unicode character format:
#UNICODE
ASCII can represent only 256 characters (general english characters and symbols) and takes 1 byte of memory. To extend representation in multiple languages UTF was made. UTF-8 (most used format of UTF) uses 4 bytes to represent a character. These 4 bytes represents all characters that you ever came across on a computer screen.
